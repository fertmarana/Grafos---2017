...
NO *criar_no(char c) { //criando um NO
    NO *no=NULL;
...
void destruir(LISTA *list){
	limpar(list); //chama  a funcao limpar para remover os elementos do list
...
void coloca_primeiro(LISTA *list, char c){
	NO *new; 
	new = criar_no(c); //cria-se um novo NO
...
void coloca_frente(LISTA *list, char c){
	NO *new = criar_no(c);//cria-se um novo NO
...
int vazio(LISTA *list){ //checa se o LISTA esta vazio a partir de seu tamanho
...
void string_para_LISTA(LISTA *list, char *word, int n){
	...
	coloca_primeiro(list,word[0]); //colocando a primeira letra ja no LISTA primeiro
	for(i=1; i<n; i++){ //enquanto nao estiver a string inteiramente preenchida
		coloca_atras(list, word[i]);//coloque as letrar uma na frente da outra 
